import numpy as np
import faiss
from src.fuzzy_matching import fuzzy_match_certification, fuzzy_match_vendor_name, fuzzy_match_industry, normalize_text as fuzzy_normalize
...
def apply_filters(meta: list[dict], filters: dict, constraints: dict) -> list[int]:
    ...
    for i, m in enumerate(meta):
        ...
        # Certifications (strict means must include all)
        if inds_wanted_certs and certs_strict:
            cert_blob = m["certifications"].lower()
            ok = ok and all(c in cert_blob for c in inds_wanted_certs)
